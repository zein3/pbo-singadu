...
const loadProblemTypes = (data) => {
  const table = document.querySelector("#data-ptype");
  let newTable = "";
  ...
  data.forEach(ptype => {
    newTable += `
            <tr>
              <td>${ptype.name}</td>
              <td>
                <span class="btn-action" onclick="openDeleteModal(${ptype.id})">
...
const getProblemTypes = async () => {
  ...
  loadProblemTypes(ptypes);
}

document.addEventListener('DOMContentLoaded', async () => {
  document.querySelector("#search").addEventListener('input', (e) => {
    const q = new RegExp(`.*${e.target.value}.*`, 'i');
    loadProblemTypes(ptypes.filter(ptype => q.test(ptype.name)));
  })
